...
<script type="text/javascript">
    ...
    function cekDetail(no_transaksi){
        ...
        $.ajax({
            type: "GET",
            url: "<?= base_url('laporankeuangan/json_detail_periksa?no_transaksi=')?>"+no_transaksi, //json get site
            dataType : 'json',
            success: function(response){
                arrData = response;
                $('#title').html('Nomor Pendaftaran : '+no_transaksi)
                for(i = 0; i < arrData.length; i++){
                    var table= '<tr><td><div class="text-center">'+arrData[i].no_pendaftaran+'</div></td>'+
                        '<td><div class="text-center">'+arrData[i].no_transaksi+'</div></td>'+
                        '<td><div class="text-center">'+arrData[i].deskripsi+'</div></td>'+
                        '<td><div class="text-center">'+arrData[i].tgl_transaksi+'</div></td>'+
