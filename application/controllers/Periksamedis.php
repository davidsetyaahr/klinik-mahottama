...
class Periksamedis extends CI_Controller
{
    ...
    function poli()
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if ($_POST['pemeriksaan_selanjutnya'] != '0') {
                ...
                $this->db->select('max(id_periksa) id_periksa');
                $getLastId = $this->db->get('tbl_periksa_lanjutan')->row();
                $lastId = $getLastId->id_periksa;
            } else {
    ...
    public function periksa_lab()
    {
        ...
        // $this->data['no_periksa'] = $data_pendaftaran->no_pendaftaran . '/' . $date_now . '/' . $data_pendaftaran->no_rekam_medis;
        $this->data['no_periksa'] = 'PRKSLAB' . '/' . $xa[1];
    ...
    public function save_periksa_lab()
    {
        ...
        //insert akuntansi
        $this->jurnal_otomatis_obat($_POST['totalObat'],0,$_POST['totalObat'],$data_transaksi['id_transaksi'],$_POST['totalObat']);
    ...
    public function operasi()
    {
        ...
        // $this->data['no_periksa'] = $data_pendaftaran->no_pendaftaran . '/' . $date_now . '/' . $data_pendaftaran->no_rekam_medis;
        $this->data['no_periksa'] = 'PRKSOPR' . '/' . $nop->no_periksa;
        $this->data['tindakan'] = $this->db->get('tbl_tindakan')->result();
    ...
    public function save_rawat_inap()
    {
        ...
        $getIdPeriksaLanjutan = $this->Periksa_model->getIdPeriksaLanjutan($this->no_pendaftaran,'2')->row(); 
            ...
            if(isset($_POST['is_tindakan_update'])){
                ...
                foreach ($getTindakanRawatInap as $key => $value) {
                    if(!in_array($value->kode_tindakan,$_POST['tindakan'])){
                        $this->db->delete('tbl_periksa_d_tindakan',['no_pendaftaran' => $this->no_pendaftaran,'tipe_periksa' => '2','kode_tindakan' => $value]);
                    }
                }
            }
        }
